...
const receiveInput = document.querySelector('.calc-input__receive .calc-input__form');
...
function selectChoose() {
  ...
  innerTextRows[2].innerHTML = `<span class="calc-text__finally-count">${0..toFixed(5)}</span> ${rotateCurrency}`;
...
menuBtn.addEventListener('click', (event) => {
  ...
  if (itemStatus == 'false') {
    ...
    item.children[0].setAttribute('style', 'transform: rotate(-45deg) translate(-5.67px, 5.67px);');
    item.children[1].setAttribute('style', 'transform: scaleX(0);');
    item.children[2].setAttribute('style', 'transform: rotate(45deg) translate(-5.67px, -5.67px);');
...
document.addEventListener('click', (event) => {
  ...
  menuListItem.forEach((item) => { 
    ...
    if ((headerBtn.dataset.active == 'active') & (event.target != headerBox) & (event.target != headerBoxTitle) & (event.target != headerBoxWrapper) & (event.target != header) & (event.target != menuItemBtn)) {
      headerBox.classList.remove('active');
      headerBox.children[1].removeAttribute('style');
      headerBtn.dataset.active = 'disactive';
    } else if (headerBtn.dataset.active == 'disactive' & (event.target == headerBtn | event.target == btnConnect | event.target == menuItemBtn | event.target == headerBtn.children[0].children[0].children[0])) {
...
calcRadio.forEach((item) => {
  ...
  item.addEventListener('click', () => {
    percent = item.value;
    let tolleranceResult = convertToReceive(selectCurrent.textContent, payInput.value, percent);
    innerTextRows[2].innerHTML = `<span class="calc-text__finally-count">${tolleranceResult.toFixed(5)}</span> ${rotateCurrency}`;
...
payInput.addEventListener('input', (event) => {
  ...
  dolarInput.forEach((item) => {
    item.textContent = "≈$" + convertToDollar(nameCurrency, target.value).toFixed(5);
  })
...
payInput.addEventListener('keydown', (event) => {
  if (!checkPhoneKey(event.key)) {
    messageError.querySelector('.calc__message-text').textContent = 'Enter a valid value (e.g. 1999.99).';
    payInput.closest('.calc-input').classList.add('form-error');
  } else {
    messageError.querySelector('.calc__message-text').textContent = 'Insufficient funds.';
    payInput.closest('.calc-input').classList.add('form-error');
  }
})
...
function checkPhoneKey(key) {
  return (key >= '0' && key <= '9') || key == '.' || key == 'Backspace' || key == 'Delete';
}
...
calcRadioInput.addEventListener('keydown', (event) => {
  if (!checkPhoneKey(event.key)) {
    messageError.querySelector('.calc__message-text').textContent = 'Enter a valid slippage percentage.';
    messageError.setAttribute('style', 'display: flex;');
  } else {
    messageError.removeAttribute('style');
  }
